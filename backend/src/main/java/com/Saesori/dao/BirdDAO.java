...
public class BirdDAO {
	...
	public List<Bird> getAllBirds() {
		...
		String sql = "SELECT id, name, description, condition_type, condition_value FROM birds";
	...
	public Bird getBirdById(int id) {
		String sql = "SELECT id, name, description, condition_type, condition_value FROM birds WHERE id = ?";
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
	...
	/**
	 * 특정 조건 타입과 값에 일치하는 새들을 조회합니다. 사용자 업적에 기반한 새 획득 로직에 사용됩니다.
	 * 
	...
	public List<Bird> getBirdsByCondition(String conditionType, int conditionValue) {
		List<Bird> birds = new ArrayList<>();
		String sql = "SELECT id, name, description, condition_type, condition_value FROM birds WHERE condition_type = ? AND condition_value <= ?";
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
