...
export default function BirdCollectionPage() {
    ...
    return (
        <div className="space-y-6">
            <h2 className="text-3xl font-display font-bold text-saesori-green-dark">My Bird Collection</h2>
            <p className="text-gray-600">Collect birds by being active! Post tweets and make friends to attract them.</p>
                    ...
                    return (
                        <div key={bird.id} className={`relative p-4 rounded-xl border-2 transition-all ${isUnlocked ? 'bg-white border-saesori-green/20 shadow-md' : 'bg-gray-100 border-dashed border-gray-300 opacity-70'}`}>
                            ...
                            <h3 className={`font-bold text-lg ${isUnlocked ? 'text-saesori-green-dark' : 'text-gray-400'}`}>
                                {isUnlocked ? bird.name : 'Unknown Bird'}
                            </h3>
                            <p className="text-sm text-gray-500 mt-1">
                                {isUnlocked ? bird.description : `Unlock: ${bird.conditionType} >= ${bird.conditionValue}`}
                            </p>
